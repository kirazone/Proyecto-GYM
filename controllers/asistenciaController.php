            ...
            case 'registrar':
                ...
                if (empty($consult)) {
                    ...
                    if ($accion == 'entrada') {
                        ...
                        // Verificar si ya existe una entrada para este estudiante hoy
                        $verificarEntrada = $asistencias->getAsistencia($fecha, $consult['id']);
                        if (!empty($verificarEntrada)) {
                            // Si ya existe una entrada, actualizamos la entrada
                            $actualizar = $asistencias->actualizarEntrada($entrada, $verificarEntrada['id']);
                            if ($actualizar) {
                                $res = array('tipo' => 'success', 'mensaje' => 'INGRESO ACTUALIZADO');
                            } else {
                                $res = array('tipo' => 'error', 'mensaje' => 'ERROR AL ACTUALIZAR');
                            }
                        } else {
                            // Si no existe una entrada, la registramos
